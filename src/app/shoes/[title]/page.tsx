...
const ShoeDetails = ({params}: ShoeDetailsProps) => {
    ...
    return (
        <section className='my-16 lg:w-10/12 px-4 mx-auto'>
            <main className="flex md:flex-row flex-col gap-4">
                <div className="flex flex-col-reverse lg:flex-row md:w-4/6 gap-5">
                    ...
                    <Image width={550} height={600} className='w-full md:h-96' src={selectedImage} alt='' />
                </div>
                <div className="flex flex-col md:w-3/6 gap-4">
                    ...
                    <div className="flex gap-2">
                        {selectedShoe?.sizes.map((size) => (
                            <button onClick={() => setSelectedSize(size)} className={`border rounded-md py-1 px-4 ${selectedSize === size ? "bg-accent text-white" : ""}`} key={size}>{size}</button>
                        ))}
                    </div>
